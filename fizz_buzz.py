...
def fizz_buzz():
	...
	inputs = map(lambda _: map(lambda num: int(num), re.sub('\n', '', _).split(' ')), file_objet.readlines())

	for input_list in inputs:
		print_words(input_list)


def print_words(input_values):
	first_divider, second_divider, count = input_values[0], input_values[1], input_values[2]
	outputs = []

	for num in range(1, count + 1):
		output = ''

		if num % first_divider != 0 and num % second_divider != 0:
			output += str(num)
		else:
			if num % first_divider == 0:
				output += 'F'
			
			if num % second_divider == 0:
				output += 'B'
